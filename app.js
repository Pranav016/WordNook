...
app.get(["/posts/:postId", "/page/posts/:postId", "/page/:page/posts/:postId", "/search/:query/posts/:postId", "/search/:query/:page/posts/:postId"], auth, function(req, res){
  const user = req.user;
  let isAuthor = false;
...
app.post("/posts/:postId/comment",auth, async function(req, res) {
...
app.post("/posts/:postId/comments/:commentNum",auth,async function(req,res){
...
//delete post route
app.post('/posts/:postId', auth, (req, res, next) => {
  const user = req.user;
  if(!user) {
    return res.status(401).redirect("/log-in");
  }
